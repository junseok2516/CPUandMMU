...
void schedule() {
    ...
    while (headNode != NULL) {
        ...
        int currPriority = 0;
        while (tmpNode != NULL) {
            if (tmpNode->task->priority > currPriority) {
                currPriority = tmpNode->task->priority;
                currTask = tmpNode->task;
            }
            tmpNode = tmpNode->next;
        }
        run(currTask, currTask->burst);

        burst[count] = currTask->burst;
        startT[count] = initialT;
        exitT[count] = initialT + currTask->burst;
        initialT += currTask->burst;
        
        delete(&headNode, currTask);
        count++;
    }
